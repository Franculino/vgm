...
__all__ = ['LinearSystemHtdTotFixedDTStemp']
...
class LinearSystemHtdTotFixedDTStemp(object):
	...
	#No flow Edges are not considered for the propagation of RBCs
        ...
        for ei in sortedE:
            ...
            #Recheck if bifurcation vertex is a noFlow Vertex (vType=7)
                ...
                if len(e['rRBC']) > 0:
                    #If borderEdge TODO needs to be uncommented
                    #if e['borderEdge']:
                    rRBCold=deepcopy(e['rRBC'])
                    #else:
                    #    rRBCold=[]
                    e['rRBC'] = e['rRBC'] + e['v'] * dt * e['sign']
                    sRBCold=deepcopy(e['sRBC'])
                    if self._stempCapRBCs:
            ...
            if e['httBC'] is not None:
                ...
                if len(rRBC) >= 1.:
                    if e['sign'] == 1:
                        ...
                        if self._stempCapRBCs:
                            e['tRBC'] = np.concatenate([np.array([None]*len(rRBC)), e['tRBC']])
                            e['sRBC'] = np.concatenate([np.array([None]*len(rRBC)), e['sRBC']])
                        vertexUpdate.append(e.target)
                        vertexUpdate.append(e.source)
                        edgeUpdate.append(ei)
                    else:
                        ...
                        if self._stempCapRBCs:
                            e['tRBC'] = np.concatenate([e['tRBC'],np.array([None]*len(rRBC))])
                            e['sRBC'] = np.concatenate([e['sRBC'],np.array([None]*len(rRBC))])
                        vertexUpdate.append(e.target)
                        vertexUpdate.append(e.source)
                        edgeUpdate.append(ei)
                if len(e['tRBC'])!=len(e['rRBC']):
